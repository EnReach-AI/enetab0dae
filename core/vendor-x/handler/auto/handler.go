...
func (h *autoHandler) Init(md md.Metadata) error {
	...
	// If enableAroAuther is true, override the auther option with ARO authenticator
	if h.md.enableAroAuther {
		aroAuther := auth_plugin.NewAROAuthenticator(h.md.backUrl,
			auth_plugin.LoggerOption(h.options.Logger.WithFields(map[string]any{
				"kind":   "auther",
				"plugin": "aro",
			})),
		)
		h.options.Auther = aroAuther
		h.options.Logger.Info("ARO auther enabled via metadata configuration")

		// Recreate child handlers with the ARO auther
		opts := []handler.Option{
			handler.AutherOption(aroAuther),
			handler.ServiceOption(h.options.Service),
			handler.RouterOption(h.options.Router),
			handler.LoggerOption(h.options.Logger),
		}

		if f := registry.HandlerRegistry().Get("http"); f != nil {
			v := append(opts,
				handler.LoggerOption(h.options.Logger.WithFields(map[string]any{"handler": "http"})))
			h.httpHandler = f(v...)
		}
		if f := registry.HandlerRegistry().Get("socks5"); f != nil {
			v := append(opts,
				handler.LoggerOption(h.options.Logger.WithFields(map[string]any{"handler": "socks5"})))
			h.socks5Handler = f(v...)
		}
	}
...
func (h *autoHandler) Handle(ctx context.Context, conn net.Conn, opts ...handler.HandleOption) error {
	...
	switch b[0] {
	case gosocks4.Ver4: // SOCKS4 - reject protocol
		log.Warn("SOCKS4 protocol is disabled, rejecting connection")
		// Send SOCKS4 rejection reply before closing
		resp := gosocks4.NewReply(gosocks4.Rejected, nil)
		if err := resp.Write(conn); err != nil {
			log.Errorf("failed to send SOCKS4 rejection reply: %v", err)
		}
		conn.Close()
		return nil
