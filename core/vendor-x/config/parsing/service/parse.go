...
func ParseService(cfg *config.ServiceConfig) (service.Service, error) {
	...
	// Prepare handler metadata early for auther parsing
	if cfg.Handler.Metadata == nil {
		cfg.Handler.Metadata = make(map[string]any)
	}
	_ = metadata.NewMetadata(cfg.Handler.Metadata) // Keep for potential future use

	// Try to parse auther with metadata to support ARO plugin
	if len(authers) == 0 && cfg.Handler.Auther != "" {
		// If no authers found from registry, check if we need to parse with metadata
		// This is a workaround for auther configs that need metadata at parse time
		handlerLogger.Debugf("attempting to parse auther '%s' with metadata", cfg.Handler.Auther)
	}
