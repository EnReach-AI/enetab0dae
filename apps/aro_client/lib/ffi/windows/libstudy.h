...
//
extern __declspec(dllexport) char* InitLibstudy(char* initParamsJSON);

// 返回：版本号字符串（C 字符串，调用方需要 free）
//
extern __declspec(dllexport) char* GetCurrentVersion(void);
extern __declspec(dllexport) char* GetLastVersion(void);
...
//
extern __declspec(dllexport) char* StartProxyWorker(char* configJSON);

// StopProxyWorker 停止代理工作节点
// 返回：JSON 格式的响应，包含成功状态和错误信息
//
extern __declspec(dllexport) char* StopProxyWorker(void);
...
//   - error: 错误信息（如果有）
//
extern __declspec(dllexport) char* GetProxyWorkerStatus(void);

// RestartProxyWorker 重启代理工作节点
// 使用之前的配置重新启动 worker
// 返回：JSON 格式的响应，包含成功状态和错误信息
//
extern __declspec(dllexport) char* RestartProxyWorker(void);

// IsProxyWorkerRunning 检查代理工作节点是否正在运行
// 返回：JSON 格式的响应，包含运行状态
//
extern __declspec(dllexport) char* IsProxyWorkerRunning(void);
